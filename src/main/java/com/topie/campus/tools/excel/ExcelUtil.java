...
public class ExcelUtil {
    ...
    private static <T> void write2SheetXScore(XSSFSheet sheet, String[] headers, Collection<T> dataset,StaticScoreDto dto, String pattern) {
        // 产生表格标题行
        XSSFRow row = sheet.createRow(0);
        for (int i = 0; i < headers.length; i++) {
            XSSFCell cell = row.createCell(i);
       ...
       cell.setCellValue(dto.getAvgScore());
       cell = row.createCell(2);
       cell.setCellValue("平均学分绩点");
       cell = row.createCell(3);
       cell.setCellValue(dto.getAvgCredit());
       int num = 4;
       for(ScoreCourceType type:dto.getScoreCourseType())
       {
    	   cell = row.createCell(num);
           cell.setCellValue(type.getCourceType());
           num++;
           cell = row.createCell(num);
           cell.setCellValue(type.getTotalCredit());
           num++;
           
       }
    } 
    
	public static <T> void exportExcelXScore(String[] headers, Collection<T> dataset,StaticScoreDto dto, OutputStream out, String pattern)
	{
			 XSSFWorkbook workBook = new XSSFWorkbook();//创建工作薄;
			    XSSFSheet sheet = workBook.createSheet();
			    write2SheetXScore(sheet, headers, dataset,dto, pattern);
			    try {
			        workBook.write(out);
			    } catch (IOException e) {
			        LG.error(e.toString(), e);
